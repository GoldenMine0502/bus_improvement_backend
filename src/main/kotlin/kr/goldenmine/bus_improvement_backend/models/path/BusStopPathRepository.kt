...
interface BusStopPathRepository : JpaRepository<BusPathInfo, Int> {
    @Query("FROM BusPathInfo as busPathInfo WHERE busPathInfo.routeNo = :routeNo")
    ...
    @Query("FROM BusPathInfo busPathInfo WHERE busPathInfo.fromId = :from AND busPathInfo.toId = :to")
